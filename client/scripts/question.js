 class q{
    ...
    create()
    {
        ...
        //newdiv.appendChild(ques);
        newdiv.appendChild(header);
        for(let i=0;i<this.options.length;i++)
        {
            let temp = document.createElement("div");
            temp.classList.add("options");
           // temp.setAttribute("onclick()","check(i)");
            temp.innerHTML=String.fromCharCode('A'.charCodeAt(0)+i)+".  "+this.options[i];
            temp.setAttribute("style","margin: 2%;");
            temp.onclick=function(){
                if(i==ans1.charCodeAt(8)-65)
                {
                    temp.setAttribute("style"," background: #00FF00");
                }else
                {
                    temp.setAttribute("style"," background: #FF0000");
                }
            }
